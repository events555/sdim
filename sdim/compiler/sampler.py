    ...
    def sample_write(
        ...
        if format == "01":
            ...
            raise NotImplementedError(
                f"Format '{format}' not implemented for sample_write."
            )
...
class CompiledDetectorSampler:
    ...
    def _parse_annotations(self):
        ...
        self.observables_meas_indices: list[
            list[int]
        ] = []  # Assuming observables are indexed 0 to N-1
        self.num_total_physical_measurements = 0
        ...
        for op_circuit in self.circuit.operations:
            op_idx_to_abs_meas_count_before_it.append(
                abs_meas_output_idx_so_far
            )
        ...
        if (
            self.num_total_measurements_from_ops
            != self.engine.num_total_measurements
        ):
            raise ValueError(
                "Mismatch in total measurement count between engine and annotation parsing."
            )
        ...
        for op_idx, op_circuit in enumerate(self.circuit.operations):
            ...
            if gate_name == "DETECTOR":
                ...
                for (
                    target
                ) in op_circuit.targets:  # These are GateTarget objects
                    if target.is_measurement_record_target:
                        abs_idx = num_meas_before_this_op + target.value
                        if not (
                            0 <= abs_idx < self.engine.num_total_measurements
                        ):
                            raise ValueError(
                                f"DETECTOR target rec[{target.value}] resolves to "
                ...
                self.detectors_meas_indices.append(
                    sorted(list(set(target_abs_indices)))
                )

            elif gate_name == "OBSERVABLE_INCLUDE":
                if not op_circuit.args:
                    raise ValueError(
                        "OBSERVABLE_INCLUDE requires a logical index argument."
                    )
                ...
                for target in op_circuit.targets:
                    if target.is_measurement_record_target:
                        ...
                        if not (
                            ...
                            raise ValueError(
                                f"OBSERVABLE_INCLUDE target rec[{target.value}] resolves to "
                                f"invalid absolute index {abs_idx} at op index {op_idx}."
                            )
                        target_abs_indices.append(abs_idx)
    ...
    def _calculate_reference_annotations(self):
        ...
        for i, indices in enumerate(self.detectors_meas_indices):
            if not indices:
                continue  # Skip empty detectors
            val = (
                sum(self.reference_sample[m_idx] for m_idx in indices)
                % self.circuit.dimension
            )
            self.ref_detector_values[i] = val

        self.ref_observable_values = np.zeros(
            self.num_observables, dtype=np.int64
        )
        for i, indices in enumerate(self.observables_meas_indices):
            if not indices:
                continue  # Skip empty observables
            val = (
                sum(self.reference_sample[m_idx] for m_idx in indices)
                % self.circuit.dimension
            )
    ...
    def sample(
        ...
        *,
        ...
        if dets_out is None and self.num_detectors > 0:
            ...
            dets_out = np.empty(
                (shots, 0), dtype=np.uint8
            )  # Handle case with no detectors
        ...
        if process_obs and obs_out is None:
            ...
            not process_obs
        ):  # Ensure obs_out is None or empty if no observables
        ...
        assert dets_out is not None
        assert obs_out is not None

        for s_idx in range(shots):
            current_shot_raw_measurements = raw_noisy_measurements_all_shots[
                s_idx, :
            ]

            if self.num_detectors > 0:
                for d_idx in range(self.num_detectors):
                    if not self.detectors_meas_indices[
                        d_idx
                    ]:  # Empty detector definition
                        dets_out[s_idx, d_idx] = (
                            0  # Or based on convention if args define value
                        )
                        continue
                    noisy_det_val = (
                        sum(
                            current_shot_raw_measurements[m_idx]
                            for m_idx in self.detectors_meas_indices[d_idx]
                        )
                        % self.engine.dimension
                    )  # Use engine's dimension
                    ref_det_val = self.ref_detector_values[d_idx]
                    dets_out[s_idx, d_idx] = (
                        1 if noisy_det_val != ref_det_val else 0
                    )
            ...
            if process_obs and obs_out is not None:
                for o_idx in range(self.num_observables):
                    ...
                    obs_out[s_idx, o_idx] = (
                        1 if noisy_obs_val != ref_obs_val else 0
                    )
    ...
    def sample_write(
        ...
        *,
        ...
        result = self.sample(shots)
        if isinstance(result, tuple):
            dets, obs = result
            if obs_out_filepath:
                with open(obs_out_filepath, "w") as f_obs:
                    for s_idx in range(shots):
                        f_obs.write("".join(map(str, obs[s_idx, :])) + "\n")
        else:
            dets = result

        with open(filepath, "w") as f_det:
            for s_idx in range(shots):
