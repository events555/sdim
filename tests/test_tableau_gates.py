...
class TestSumGate:
    def test_symbolic(self):
        (zc_i, xc_i, zt_i, xt_i, zc_j, xc_j, zt_j, xt_j) = symbols(
...
class TestCZGate:
    def test_symbolic(self):
        (z1_i, x1_i, z2_i, x2_i, z1_j, x1_j, z2_j, x2_j) = symbols(
...
class TestMultiplyGate:
    def test_symbolic(self):
        z_i, x_i, z_j, x_j, a, a_inv = symbols("z_i x_i z_j x_j a a_inv")
        before = z_i * x_j - x_i * z_j
...
class TestProductAndPowerRules:
    ...
    def test_power_rule_matches_iterated_product(self, c):
        ...
        for _ in range(c):
            acc_phi, acc_z, acc_x = product(acc_phi, acc_z, acc_x, phi, z, x)
