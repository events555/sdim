...
Tests two layers:
  1. Low-level: individual _sample_* methods produce correct distributions.
...
def test_x_error(noise_circuit, prob):
    ...
    samples = noise_circuit._sample_x_error(N_SHOTS, prob)
    ...
    if prob > 0:
        ...
        if d > 2 and prob > 1e-9:
            ...
            for k in range(1, d):
                assert counts[(k, 0)] / N_SHOTS == pytest.approx(expected_each, abs=PROB_TOL)
...
def test_z_error(noise_circuit, prob):
    d = noise_circuit.dimension
    samples = noise_circuit._sample_z_error(N_SHOTS, prob)
    counts = Counter(pauli_to_tuple(s) for s in samples)
    ...
    if prob > 0:
        ...
        if d > 2 and prob > 1e-9:
            expected_each = prob / (d - 1)
            for k in range(1, d):
                assert counts[(0, k)] / N_SHOTS == pytest.approx(expected_each, abs=PROB_TOL)
...
def test_y_error(noise_circuit, prob):
    d = noise_circuit.dimension
    samples = noise_circuit._sample_y_error(N_SHOTS, prob)
    counts = Counter(pauli_to_tuple(s) for s in samples)
    ...
    if prob > 0 and d == 2:
        ...
        expected_each = prob / (d - 1)
        for k in range(1, d):
            assert counts[(k, k)] / N_SHOTS == pytest.approx(expected_each, abs=PROB_TOL)
...
def test_depolarize1(noise_circuit, prob):
    d = noise_circuit.dimension
    samples = noise_circuit._sample_depolarize1_noise(N_SHOTS, prob)
    counts = Counter(pauli_to_tuple(s) for s in samples)
...
def test_depolarize2(noise_circuit, prob):
    d = noise_circuit.dimension
    samples = noise_circuit._sample_depolarize2_noise(N_SHOTS, prob)
    counts = Counter(pauli_to_tuple(s) for s in samples)

    assert counts[(0, 0, 0, 0)] / N_SHOTS == pytest.approx(1 - prob, abs=PROB_TOL)
...
def test_heralded_erasure(noise_circuit, prob):
    d = noise_circuit.dimension
    pauli_samples, erasure_flags = noise_circuit._sample_heralded_erasure_noise(N_SHOTS, prob)
        ...
        if erased.size > 0:
            ...
            for x_exp in range(d):
                for z_exp in range(d):
                    ...
                    assert observed == pytest.approx(expected_each, abs=scaled_tol)
...
def test_pauli_channel1(noise_circuit, pmf_choice):
    ...
    samples = noise_circuit._sample_pauli_channel1_noise(N_SHOTS, list(pmf))
    counts = Counter(pauli_to_tuple(s) for s in samples)

    for key, expected_p in expected.items():
        observed = counts[key] / N_SHOTS
        tol = PROB_TOL
        if expected_p < PROB_TOL * 2:
            tol = expected_p * 0.5 if expected_p > 1e-9 else PROB_TOL
...
def test_depolarize1_circuit_distribution(
    dimension, error_prob, num_noisy_qubits,
    initial_state_is_zero, measure_Z_basis,
):
    d = dimension
    n = 3
    ...
    for q_idx in range(n):
        ...
        expected = expected_probs_qudit(d, p, initial_state_is_zero, measure_Z_basis)
